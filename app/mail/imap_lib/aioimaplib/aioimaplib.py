...
class IMAP4ClientProtocol(asyncio.Protocol):
    ...
    def _handle_responses(self, data: bytes, line_handler: Callable[[bytes, Command], Optional[Command]],
    ...
    async def fetch(self, message_set: str, message_parts: str, by_uid: bool = False,
    ...
    async def uid(self, command: str, *criteria: str, timeout: float = None) -> Response:
        ...
        if command.upper() == 'THREAD':  # <-- Добавляем поддержку UID THREAD
            return await self.execute(
                Command('THREAD', self.new_tag(), *criteria, prefix='UID', loop=self.loop, timeout=timeout))
        raise Abort(
            'command UID only possible with COPY, FETCH, EXPUNGE (w/UIDPLUS) or STORE (was %s)' % command.upper())
    ...
    async def append(self, message_bytes: bytes, mailbox: str = 'INBOX', flags: str = None, date: Any = None,
                     timeout: float = None) -> Response:
        args = [mailbox]
...
class IMAP4:
    ...
    async def rename(self, old_mailbox_name: str, new_mailbox_name: str) -> Response:
        return await asyncio.wait_for(self.protocol.simple_command('RENAME', old_mailbox_name, new_mailbox_name),
                                      self.timeout)

    async def getquotaroot(self, mailbox_name: str) -> Response:
        return await asyncio.wait_for(self.protocol.execute(
            Command('GETQUOTAROOT', self.protocol.new_tag(), 'INBOX', untagged_resp_name='QUOTA')), self.timeout)
